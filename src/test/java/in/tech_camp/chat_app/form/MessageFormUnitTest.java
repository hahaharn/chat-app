...
public class MessageFormUnitTest {
  ...
  public void setUp() {
    ...
    //エラーを蓄積するbindingResultの身代わり（モック）を用意
  }
  ...
  class メッセージが投稿できる場合 {
    ...
    public void contentが空でも保存できる () {
      messageForm.setContent(""); //文章を空にするが画像は入っている状態
      messageForm.validateMessage(bindingResult); //バリデーション実行
      verify(bindingResult, never()).rejectValue(anyString(), anyString(), anyString());  //答え合わせ
    }
